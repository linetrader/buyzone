// src/app/[locale]/(site)/tree/sponser/page.tsx
...
import { User, BadgeCheck, Calendar, RefreshCw, Users } from "lucide-react"; // Users 아이콘 사용

// --- Custom Node 데이터 타입 정의 (Referrer/Sponsor 공유) ---
interface NodeData {
  label: string;
  level: number;
  code: string;
...
// --- 2. Dagre 레이아웃 설정 (ReferrerTreePage에서 복사) ---
...
// --- 더미 데이터 (ReferrerTreePage에서 복사) ---
const INITIAL_NODES: Node<NodeData>[] = [
  { id: '1', type: 'customNode', data: { label: 'Root Sponsor', level: 5, code: 'MASTER', joinDate: '2024-01-01' }, position: { x: 0, y: 0 } },
  { id: '2', type: 'customNode', data: { label: 'Sponsee A', level: 3, code: 'PARTNER_A', joinDate: '2024-02-15' }, position: { x: 0, y: 0 } },
  { id: '3', type: 'customNode', data: { label: 'Sponsee B', level: 2, code: 'PARTNER_B', joinDate: '2024-03-10' }, position: { x: 0, y: 0 } },
  { id: '4', type: 'customNode', data: { label: 'Sub Sponsee A-1', level: 1, code: 'USER_A1', joinDate: '2024-04-05' }, position: { x: 0, y: 0 } },
];
...
// --- 3. 메인 페이지 컴포넌트 ---
export default function SponserTreePage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      // ✅ [수정] API 경로 변경
      const res = await fetch("/api/tree/sponser"); 
      ...
      {showEmptyMessage && (
        <div className="absolute inset-0 z-10 flex items-center justify-center">
          <div className="text-center text-gray-500 p-10 bg-white rounded-xl shadow-lg border border-gray-200">
            ...
            <h2 className="text-lg font-bold">후원 조직도 데이터가 없습니다.</h2>
            <p className="text-sm mt-1">후원 구조는 별도의 배치 규칙에 따릅니다.</p>
          </div>
        </div>
      )}
      ...
      >
        ...
        <Panel position="top-left" className="m-4">
          <div className="bg-white/90 backdrop-blur px-5 py-4 rounded-2xl shadow-lg border border-gray-200">
            <h1 className="text-xl font-extrabold text-gray-900">후원 조직도</h1>
